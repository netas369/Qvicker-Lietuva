        ...
        <div class="mx-auto">
            ...
            <div class="sm:hidden py-2">
                <select id="mobileNavigation" onchange="navigateTo(this.value)" class="block w-full rounded-md border-gray-300 py-2 pl-3 pr-10 text-base focus:border-primary focus:outline-none focus:ring-primary">
                    <option value="{{ route('profile.dashboard') }}" {{ Route::currentRouteName() == 'profile.dashboard' ? 'selected' : '' }}>
                        Apžvalga
                    </option>
                    <option
                        value="{{ route('myprofile') }}" {{ Route::currentRouteName() == 'myprofile' ? 'selected' : '' }}>
                        Profilis
                    </option>

                    @if($user && $user->role === 'provider')
                        <option
                            value="{{ route('provider.work') }}" {{ Route::currentRouteName() == 'provider.work' ? 'selected' : '' }}>
                            Darbas
                        </option>
                        <option
                            value="{{ route('provider.calendar') }}" {{ Route::currentRouteName() == 'provider.calendar' ? 'selected' : '' }}>
                            Kalendorius
                        </option>
                        <option
                            value="{{ route('reservations.provider') }}" {{ Route::currentRouteName() == 'reservations.provider' ? 'selected' : '' }}>
                            Gautos užklausos
                        </option>
                    @else
                        <option
                            value="{{ route('reservations.seeker') }}" {{ Route::currentRouteName() == 'reservations.seeker' ? 'selected' : '' }}>
                            Mano užklausos
                        </option>
                    @endif
                </select>
